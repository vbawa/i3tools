...
def switch_mode(
    ...
    key = mode_str.upper()
    matched_modes = [m for m in Mode.__dict__ if m.startswith(key)]
    assert len(matched_modes) <= 1, f"Mode string {mode_str} prefix-matched multiple modes: {matched_modes}"
    ws_ids_to_disable = (
        DISABLED_WS_IDS_BY_MODE[Mode[matched_modes[0]]]
